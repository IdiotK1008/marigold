...
from .src.util.dmvn_resnet import DepthMeanVarianceNet
...
if "__main__" == __name__:
    ...
    parser.add_argument(
        "--checkpoint",
        type=str,
        default="checkpoint/my-model-v3-fisheye",
        help="Checkpoint path or hub name.",
    )
    parser.add_argument(
        "--dmvn_checkpoint",
        type=str,
        # default="./base_ckpt_dir/resnet18/depth_mean_variance_net.pth",
        default=None,
        help="Checkpoint path or hub name.",
    )
    ...
    parser.add_argument(
        "--dataset_config",
        type=str,
        default="./config/dataset/data_kitti360_train.yaml",
    ...
    parser.add_argument(
        "--denoise_steps",
        type=int,
        default=30,  # quantitative evaluation uses 50 steps
    ...
    if dmvn_path is not None:
        dmvn = DepthMeanVarianceNet(input_channels=4)
    ...
    with torch.no_grad():
        for batch in tqdm(
            ...
            depth_gt = batch["depth_raw_linear"].squeeze().numpy()
            depth_gt_mask = batch["valid_mask_raw"].squeeze().numpy()
            depth_gt_mean = np.mean(depth_gt[depth_gt_mask])
            depth_gt_variance = np.var(depth_gt[depth_gt_mask])
            #print(f"depth_gt_mean: {depth_gt_mean}, depth_gt_variance: {depth_gt_variance}")

            # 将数据*1000后转换为整型
            depth_gt_mean_1000 = int(round(depth_gt_mean * 1000))
            depth_gt_variance_1000 = int(round(depth_gt_variance * 1000))

            # Save predictions
            rgb_filename = batch["rgb_relative_path"][0]
            rgb_basename = os.path.basename(rgb_filename).split(".")[0] + "_" + str(depth_gt_mean_1000) + "_" + str(depth_gt_variance_1000) + ".npy"
