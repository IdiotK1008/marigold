...
if "__main__" == __name__:
    ...
    with torch.no_grad():
        ...
        for rgb_path in tqdm(rgb_filename_list, desc="Estimating depth", leave=True):
            ...
            pred_name_abs = pred_name_base + "_abs"
            abs_npy_save_path = os.path.join(output_dir_npy, f"{pred_name_abs}.npy")
            if os.path.exists(abs_npy_save_path):
                logging.warning(f"Existing file: '{abs_npy_save_path}' will be overwritten")
            np.save(abs_npy_save_path, abs_depth_pred)
