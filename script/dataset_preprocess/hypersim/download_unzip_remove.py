...
parser.add_argument("--downloads_dir", type=str, default="data/Hypersim/raw_data")
...
def download(url):
    ...
    # unzip
    # cmd = f'unzip -j {download_file} "*color.hdf5" "*depth_meters.hdf5" "*render_entity_id.hdf5" -d {args.downloads_dir}'
    cmd = "unzip " + download_file + " -d " + args.downloads_dir
    print("")
    print(cmd)
    print("")
    retval = os.system(cmd)
    ...
    # remove
    unzip_dir = download_file.split('.')[0]
    for dirpath, dirnames, filenames in os.walk(os.path.join(unzip_dir, 'images')):
        ...
        for filename in filenames:
            ...
            if parts[-2] != 'color' and parts[-2] != 'depth_meters' and parts[-2] != 'render_entity_id':
...
st = 10 * args.turn
this_turn = urls_to_download[st:]
for url in this_turn:
    download(url)

# next:6
'''url = "https://docs-assets.developer.apple.com/ml-research/datasets/hypersim/v1/scenes/ai_001_003.zip"
download(url)'''
