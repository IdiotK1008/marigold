...
if "__main__" == __name__:
    ...
    parser.add_argument(
        "--dataset_config",
        type=str,
        default="config/dataset/dataset_train.yaml",
    ...
    with torch.no_grad():
        for batch in tqdm(
            ...
            if depth_pred.ndim == 4:
                depth_pred = depth_pred.mean(axis=0)
                depth_pred = depth_pred.squeeze()
            # 检查depth_pred的第一维度是否为 4
            if depth_pred.ndim != 3 or depth_pred.shape[0] != 4:
            ...
            pred_basename = rgb_basename.replace("rgb", "pred")
